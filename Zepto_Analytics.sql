...
CREATE database Zepto_Quick_Commerce;
USE  Zepto_Quick_Commerce;
...
SELECT SUM(quantity) as total_quanity_sold from transactions;
SELECT count(distinct Order_ID) as total_orders from orders;
## average order value
SELECT SUM(t.amount) / COUNT(DISTINCT o.Order_ID) AS Average_Order_value 
FROM transactions t 
...
SELECT
CASE WHEN d.delivery_time_mins =30 THEN "Under 30 mins"
...
SELECT p.Product_ID,
  ...
  ROUND(SUM(t.amount)/SUM(DISTINCT quantity),2) AS revenue_per_unit
  FROM products p JOIN transactions t
  ON p.Product_ID = t.product_id
GROUP BY p.Product_ID,
       p.product_name,
...
SUM(t.amount) AS Revenue,
ROUND(SUM(t.amount)/COUNT(DISTINCT o.Order_ID),2) AS AOV
FROM customers c JOIN orders o
ON c.Customer_ID = o.customer_id
JOIN transactions t
ON o.Order_ID = t.order_id
...
SELECT DAYNAME(order_date) AS Week,
...
ROUND(
     ...
     *100/ LAG(Revenue) OVER(ORDER BY Order_Month),2)
     AS MOM
...
SELECT 
CASE WHEN amount < 200 THEN "Under 200"
...
SELECT YEAR(created_date) AS Signup_Year,
...
c.Customer_ID,
COUNT(DISTINCT o.Order_ID) AS order_count,
CASE WHEN COUNT(DISTINCT o.Order_ID) = 0 THEN "No orders"
WHEN COUNT(DISTINCT o.Order_ID) = 1 THEN "One Time"
WHEN COUNT(DISTINCT o.Order_ID) BETWEEN 2 AND 4 THEN "Repeat"
ELSE "Frequent"
END AS Customer_Segment
...
# Do Higher-Value Orders Get Better Ratings?
    SELECT
    CASE WHEN t.amount < 200 THEN "Under 200"
